...
def load_ocr_model():
    ...
    processor_path = "basenet/ocr_weights"

    if not os.path.exists(processor_path):
        st.error(f"OCR processor weights not found at: {processor_path}")
        st.stop()
...
# Add this information to the sidebar
def show_model_info():
    st.sidebar.markdown("---")
    st.sidebar.markdown("### Model Information")
    st.sidebar.info("Using CPU-optimized quantized ONNX model")

# Call this after loading the model, just before the main UI code
show_model_info()
