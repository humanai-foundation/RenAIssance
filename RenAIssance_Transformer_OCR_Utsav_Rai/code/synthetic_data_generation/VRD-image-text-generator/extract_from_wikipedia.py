...
def extract_from_wikipedia(nb_visited_pages, max_chunk_size, output_path,pattern=r"\[\d+\]"):
    wikipedia_pages = []

    for _ in range(nb_visited_pages):
        page = requests.get("https://es.wikipedia.org/wiki/Special:Random", timeout=5)
        soup = BeautifulSoup(page.content, 'html.parser')
        p = soup.find_all('p')
        text = p[1].get_text() if len(p) > 1 else ""

        max_retries = 5
        retries = 0

        while (len(text) < 50 or "\\" in text.replace("\n","").replace("\u200b","")) and retries < max_retries:
            page = requests.get("https://es.wikipedia.org/wiki/Special:Random", timeout=5)
            soup = BeautifulSoup(page.content, 'html.parser')
            p = soup.find_all('p')
            text = p[1].get_text() if len(p) > 1 else ""
